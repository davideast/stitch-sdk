...
| --- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| 00  | `v1/00-d12-options-object`      | Commit the already-implemented D12 work (codegen options-object, regen, tests, docs).                                                                                                                                                                                                                      | pending |
| 01  | `v1/01-deterministic-codegen`   | §0.3a: relative-path hashing in `generate-sdk.ts`/`validate-generated.ts`; remove `Generated:` timestamp from emitted headers; `generatedAt` only in lock; recompute stale `lock.manifest.sourceHash` from the committed manifest (note: true re-capture needs live server — see needs-human); atomic manifest+lock writes; loud failure on corrupt lock. | pending |
| 02  | `v1/02-ci-and-packaging`        | §0.3b+0.4: CI adds `validate:generated` + `test:scripts` jobs; delete phantom root scripts, implement `scripts/sync-versions.ts`; `@google/adk` + `ai` → optional peers with guarded import & actionable error; exports `types` first; `publint` added to publish-readiness.                                  | pending |
| 03  | `v1/03-entity-manager`          | §0.1: composite cache keys (all reference keys), generated `static entityKey`, remove `"unknown"` fallback (uncached instance + debug warn), license header; full test matrix incl. cross-project collision regression.                                                                                      | pending |
| 04  | `v1/04-construction-paths`      | §0.2 (amended): `upload-handler` + `proxy/virtual-tools` route through `entities.resolve`; constructors throw descriptive error on string data pointing to factories; dummyClient returns parsed payload + propagates `isError`; de-mocked tests for both seams.                                              | pending |
| 05  | `v1/05-handler-hardening`       | §0.5: `allSettled`-style concurrency pool with per-asset error report (`success:false` on partial), `r.ok` on HTML fetch, tmp-file cleanup on failure, sanitize `ext`, https-only asset URLs, delete dead comment/codes.                                                                                      | pending |

### Stage B — Pipeline (Phase 1; internal)

| #   | Branch                       | Scope (plan refs)                                                                                                                                                                                                                                              | Status  |
| --- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| 06  | `v1/06-codegen-snapshots`    | §1.1 (amended): fixture manifest + domain-map covering every IR feature; small per-feature snapshots; **compile + behaviorally test** fixture output against a fake client; replace grep-style tests.                                                                | pending |
| 07  | `v1/07-ir-strict`            | §1.2+1.3: `.strict()` IR; delete `ProjectionStep.fallback` + `FieldMappingSpec`; implement `ArgParam.default`; reject array-step without `index`/`each`/`find` (validation==emission); validate `find` dot-paths; unbounded-array lint (warn) + escape hatch; cache-projection `index` support. | pending |
| 08  | `v1/08-capture-raw-skills`   | §1.5+1.6: capture writes raw schemas, repair at load; lock `repairs` section; generate IR reference section of `stitch-sdk-domain-design` SKILL from `ir-schema.ts` (`.describe()` everything); CI no-diff check; purge stale `fieldMapping`/`idField` docs.        | pending |
| 09  | `v1/09-response-fixtures`    | §1.7: `test/fixtures/responses/` per tool — synthesized from `outputSchema` incl. **multi-screen** shapes (live refresh script for when a key is available); per-binding projection tests against fixtures.                                                          | pending |

### Stage C — Runtime (Phase 2; mostly internal)

| #   | Branch                        | Scope (plan refs)                                                                                                                                                                                                                                                  | Status  |
| --- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| 10  | `v1/10-errors-and-retry`      | §2.2+2.3 (D6 revised): central `classifyError` (status-first, word-boundary fallback), adopt in all 4 sites, dead codes removed, matrix tests; retry/backoff for **idempotent reads only** (`get_*`/`list_*`), `Retry-After`, config knob, fake-timer tests.            | pending |
